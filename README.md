...
│   └── vite.config.js
├── data/
│   └── byteme.db                   ← SQLite database (auto-created)
...
| Database     | **SQLite** (via Python stdlib) | Versioned iteration/rule storage      |
...
Each iteration is stored in SQLite and visible in **Iteration History**.
...
### 5. Rule Versioning
All rules, itemsets, thresholds, and recommendation outputs are stored per iteration in SQLite. You can query any past iteration.
...
4. **Storage**: "Every iteration is versioned in SQLite so we can compare patterns over time and track business trends."
